...
cat > .env << EOF
DATABASE_URL=postgresql://postgres:Post_1234@localhost:5432/remotriage
...
# Frontend automatically detects backend from .env
# Development: http://localhost:8000 (default in .env)
...
|----------|-------------|---------|
| `DATABASE_URL` | PostgreSQL connection | `postgresql://postgres:password@localhost:5432/remotriage` |
...
| Variable | Description | Example |
|----------|-------------|---------|
| `VITE_API_URL` | Backend API URL | `http://localhost:8000` (dev) or `https://api.remotriage.app` (prod) |
...
Open http://localhost:5173 in your browser and test the form.
...
│   │   │   ├── voice.py            # Voice calls
│   │   │   └── prognosis.py        # Case lookup
│   │   ├── services/
...
1. **Development workflow**:
   ...
   # Terminal 3: Monitor PostgreSQL (optional)
   psql remotriage
   ```
...
2. **Test with curl**:
   ...
   curl -X POST http://localhost:8000/triage/ \
     -H "Content-Type: application/json" \
     -d '{"symptoms":"fever","location":"Nairobi"}'
   ```
